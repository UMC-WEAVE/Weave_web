...
function showMapModal(param) {
    ...
    function placesSearchCB(data, status, pagination) {
        if (status === kakao.maps.services.Status.OK) {
            ...
            //지도 세팅 : 레벨 3으로 고정, 중심좌표는 결과값의 1번 위도경도값
    ...
    function getListItem(index, places) {
        ...
        itemStr += '<input type="button" value="추가하기" name="' + places.road_address_name + '" class="addBtn">' + '</div>';//name속성을 통해 주소전달

        el.innerHTML = itemStr;
        el.className = 'item';

        el.onclick = function () {
            console.log(places.y, places.x);
